    ...
    <div className="overflow-x-auto">
      <table className="w-full">
        ...
        {contacts.map((msg) => (
          <tbody key={msg._id}>
            <tr
              className={`border-b border-gray-100 cursor-pointer hover:bg-sakura-50/50 ${!msg.read ? 'bg-sakura-50/20' : ''} ${expanded === msg._id ? 'bg-sakura-50/30' : ''}`}
              onClick={() => { setExpanded(expanded === msg._id ? null : msg._id); setChatOpenId(null); }}
            >
              <td className="py-3 px-4 text-sm font-mono text-gray-600">
                <div className="flex items-center gap-1.5">
                  <svg
                    className={`w-3.5 h-3.5 text-gray-400 transition-transform flex-shrink-0 ${expanded === msg._id ? 'rotate-90' : ''}`}
              ...
              <td className="py-3 px-4">
                <div className="text-sm font-medium text-dark">{msg.name}</div>
                <div className="text-xs text-gray-500">{msg.email}</div>
              </td>
              <td className="py-3 px-4 text-sm text-gray-700 max-w-[200px]">
                <span className="truncate block">{msg.subject || <span className="text-gray-300">—</span>}</span>
              </td>
              <td className="py-3 px-4" onClick={(e) => e.stopPropagation()}>
                <select
                  value={msg.status || 'pending'}
                  onChange={(e) => handleStatusChange(msg._id, e.target.value)}
                  className={`text-xs font-medium px-2 py-1 rounded-full border-0 cursor-pointer ${statusColors[msg.status] || statusColors.pending}`}
                >
                  ...
                  <div className="flex flex-col md:flex-row gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="bg-gray-50 rounded-lg p-3 space-y-2 text-sm">
                        {msg.subject && (
                          <div>
                            <span className="font-medium text-dark">Subject:</span>{' '}
                        ...
                        {msg.message && (
                          <div>
                            ...
                            <p className="text-gray-600 whitespace-pre-wrap mt-0.5">{msg.message}</p>
                          </div>
                        )}
                    ...
                    <div className="md:w-72 flex-shrink-0">
                      <button
                        onClick={(e) => { e.stopPropagation(); setChatOpenId(chatOpenId === msg._id ? null : msg._id); }}
                        className="flex items-center gap-2 mb-2 w-full text-left group"
                      >
                        <h4 className="text-xs font-semibold text-gray-500 uppercase">Chat</h4>
                        {msg.replies?.length > 0 && (
                          <span className="text-xs text-gray-400">({msg.replies.length})</span>
                        )}
                        <svg
                          className={`w-3.5 h-3.5 text-gray-400 ml-auto transition-transform ${chatOpenId === msg._id ? 'rotate-90' : ''}`}
                          fill="none" stroke="currentColor" viewBox="0 0 24 24"
                        >
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                        </svg>
                      ...
                      {chatOpenId === msg._id && <div className="space-y-2">
                        ...
                        {replyingTo === msg._id ? (
                          ...
                          <button
                            onClick={(e) => { e.stopPropagation(); setReplyingTo(msg._id); }}
